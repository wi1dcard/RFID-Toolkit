...
begin
  if cmdRet <> 0 then
  begin
    s := ' “' + cmdName + '” 指令返回=0x' + IntToHex(cmdRet, 2) + '(' + UHFReader09_GetReturnCodeDesc(cmdRet) + ')';
    if ErrorCode <> -1 then
    begin
...
begin
  ...
  if fCmdRet = 0 then
   returninfo:=returninfo+setinfo+'功率dbm成功'
  else if fCmdRet=RecmdErr then
  returninfo:=returninfo+setinfo+'功率dbm返回指令错误'
  else
  begin
  returninfo:=returninfo+setinfo+'功率dbm失败';
...
begin
  ...
  AddCmdLog('SetMultiReadProtect_G2', '设置单张读保护（不需卡号）', fCmdRet,ferrorcode);
  if fCmdRet=0 then
  begin
  StatusBar1.Panels.Items[0].text := FormatDateTime('hh:mm:ss', Now) +  ' “设置单张读保护（不需卡号）”指令返回=0x00' +
            '(设置单张读保护（不需卡号）成功)'
  end;
end;
...
begin
    ...
    AddCmdLog('WriteData','块写数据', fCmdRet,ferrorcode);
    if fCmdRet=0 then
    begin
      StatusBar1.Panels.Items[0].text := FormatDateTime('hh:mm:ss', Now) +  ' “块写数据”指令返回=0x00' +
                '(数据完全写入成功)';
    end;
end;
...
begin
  ...
  AddCmdLog('BuzzerAndLEDControl','设置', fCmdRet);
end;
