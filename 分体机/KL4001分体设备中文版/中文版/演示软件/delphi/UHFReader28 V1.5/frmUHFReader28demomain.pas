...
begin
  if cmdRet <> 0 then
  begin
    s := ' “' + cmdName + '” 指令返回=0x' + IntToHex(cmdRet, 2) + '(' + UHFReader28_GetReturnCodeDesc(cmdRet) + ')';
    if ErrorCode <> -1 then
    begin
...
procedure TfrmUHFReader28main.Action_SelfTestModeExecute(Sender: TObject);
var
...
begin
   ...
   AddCmdLog('SelfTestMode', TAction(Sender).Caption+'自测试模式', fCmdRet);
end;
...
procedure TfrmUHFReader28main.Action_RfOutputExecute(Sender: TObject);
var
  onoff:Byte;
begin
...
AddCmdLog('SetAntennaMultiplexing', '天线设置', fCmdRet);
end;
...
begin
  ...
  begin
    Application.MessageBox('请输入数据', '信息提示', MB_ICONINFORMATION);
    exit;
  end;
  ...
  SetTime[5] := char(strtoint(copy(Edit_Sedcond.Text, 1, 2)));
    ...
    begin
      Application.MessageBox('请输入00-99之间的任一数值', '信息提示', MB_ICONINFORMATION);
      exit;
    end;
    fCmdRet := SetReal_timeClock(fComAdr, SetTime,frmComPortindex);
    AddCmdLog('SetReal_timeClock', '设置实时时钟', fCmdRet);
  end
  ...
  begin
    ...
    AddCmdLog('GetTime', '查询实时时钟', fCmdRet);
  end;
end;
...
begin
  ...
  begin
  MessageBox(0, '请输入正确的数据！', '提示', MB_ICONWARNING or MB_OK);
  Exit;
  end;
...
begin
   ...
   begin
     ...
     if(RadioButton12.Checked)then Protocol:=$11;
   end
   else
    Protocol:=$80; //
...
begin
  ...
  begin
    ...
    Edit10.Text:=temp;
    ComboBox5.ItemIndex:= TriggerTime;
    Edit4.Text:=IntToHex(AdrTID,2);
    Edit5.Text:=IntToHex(LenTID,2);
...
//  EnableFlag:=True;
 ...
 // Sleep(100);
 // EnableFlag:=false;;
  AddCmdLog('GetTagBufferInfo', '获取存储块标签信息', fCmdRet);
end;
...
procedure TfrmUHFReader28main.ComboBox4Change(Sender: TObject);
begin
;
end;
...
begin
  ...
  begin
  MessageDlg('TID参数错误', mtWarning, [mbOK], 0);
  exit;
  end;
...
procedure TfrmUHFReader28main.Edit_WriteDataKeyPress(Sender: TObject;
  var Key: Char);
var L:Boolean;
begin
...
procedure TfrmUHFReader28main.RadioButton13Click(Sender: TObject);
begin
...
begin
  ...
  begin
    Application.MessageBox('通讯错误!', '信息', MB_ICONINFORMATION);
  end;
...
procedure TfrmUHFReader28main.RadioButton14Click(Sender: TObject);
begin
if(ComboBox_AlreadyOpenCOM.Items.Count>0)then
Button4Click(Sender) ;
...
begin
  if(Edit16.Text='')then
  begin
  MessageDlg('AT指令不能为空!', mtWarning, [mbOK], 0);
  Exit;
  end;
...
begin
  ...
  if(Pos('ERROR',recv)>0)or(recvLen=0)then
   begin
      MessageDlg('读取失败', mtWarning, [mbOK], 0);
      SeriaATflag:=True;
      Exit;
   end;
   m:= Pos(',',recv);
