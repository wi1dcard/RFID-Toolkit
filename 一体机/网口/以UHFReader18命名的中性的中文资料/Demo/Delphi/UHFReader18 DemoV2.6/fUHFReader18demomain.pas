...
type
  TfrmUHFReader18demomain = class(TForm)
    ...
    procedure FormCreate(Sender: TObject); //串口号初始化列表函数
...
begin
  if cmdRet <> 0 then
  begin
    s := ' “' + cmdName + '” 指令返回=0x' + IntToHex(cmdRet, 2) + '(' + UHFReader18_GetReturnCodeDesc(cmdRet) + ')';
    if ErrorCode <> -1 then
    begin
...
begin
  ...
  returninfoDlg:=returninfoDlg+#13#10+setinfo+'功率dbm失败指令返回=0x'
       +inttostr(fCmdRet)+'('+UHFReader18_GetReturnCodeDesc(fCmdRet)+')';
  end;
  ...
  returninfoDlg:=returninfoDlg+#13#10+setinfo+'频率失败指令返回=0x'
       +inttostr(fCmdRet)+'('+UHFReader18_GetReturnCodeDesc(fCmdRet)+')';
  end;
  ...
  returninfoDlg:=returninfoDlg+#13#10+setinfo+'波特率失败指令返回=0x'
       +inttostr(fCmdRet)+'('+UHFReader18_GetReturnCodeDesc(fCmdRet)+')';
  end;
  ...
  returninfoDlg:=returninfoDlg+#13#10+setinfo+'询查时间失败指令返回=0x'
       +inttostr(fCmdRet)+'('+UHFReader18_GetReturnCodeDesc(fCmdRet)+')';
  end;
...
begin
  ...
  Exit ;
    ...
    begin
      ...
      AddCmdLog('EraseCard', '擦除数据', fCmdRet,ferrorcode);
      if fCmdRet=0 then
      begin
...
begin
    ...
    AddCmdLog('SetCardProtect', '设置保护', fCmdRet,ferrorcode);
end;
...
begin
  ...
  begin
    ...
    AddCmdLog('DestroyCard', '销毁标签', fCmdRet,ferrorcode);
    if fCmdRet=0 then
    begin
...
begin
  ...
  AddCmdLog('SetReadProtect_G2','设置读保护',fCmdRet,ferrorcode);
  if fCmdRet=0 then
  begin
...
begin
  ...
  AddCmdLog('SetMultiReadProtect_G2', '设置单张读保护（不需EPC号）', fCmdRet,ferrorcode);
  if fCmdRet=0 then
  begin
    StatusBar1.Panels.Items[0].text := FormatDateTime('hh:mm:ss', Now) +  ' “设置单张读保护（不需EPC号）”指令返回=0x00' +
            '(设置单张读保护（不需EPC号）成功)'
  end;
end;
...
begin
  ...
  AddCmdLog('RemoveReadProtect_G2', '解锁读保护', fCmdRet,ferrorcode );
  if fCmdRet=0 then
  begin
...
begin
  ...
  AddCmdLog('CheckReadProtected_G2','检测读保护', fCmdRet);
  if fCmdRet=0 then
  begin
   if readpro=0 then
...
begin
  ...
  AddCmdLog('SetEASAlarm_G2', '报警设置', fCmdRet,ferrorcode);     //v2.1改
  if fCmdRet=0 then
  begin
...
begin
     if(Edit2.Text='')or(Edit3.Text='')then
      ...
      AddCmdLog('LockUserBlock_G2', '用户区数据块锁定', fCmdRet,ferrorcode);
      if fCmdRet=0 then
      begin
...
begin
  ...
  try
    ...
    begin
      ...
      AddCmdLog('ReadCard', '读数据', fCmdRet);
    ...
    begin
      ...
      AddCmdLog('WriteCard', '写数据', fCmdRet);
...
begin
  ...
  fCmdRet:=SetWGParameter(fComAdr,Wg_mode,Wg_Data_Inteval,Wg_Pulse_Width,Wg_Pulse_Inteval,frmComPortindex)  ;
   AddCmdLog('Set Parameter', '韦根设置', fCmdRet);
end;
...
begin
  ...
  if(RadioButton9.Checked)then
   ...
   if(Edit1.Text='')then
   begin
      MessageDlg('起始地址不能为空！', mtInformation, [mbOK], 0);
      Exit;
   end;
    ...
    AddCmdLog('Set Parameter', '模式设置', fCmdRet);
end;
...
begin
   ...
   AddCmdLog('Set Parameter', '获取工作模式参数', fCmdRet);
end;
 ...
 if(RadioButton5.Checked)then
  begin
   ...
   begin
    ...
    RadioButton18.Enabled:=True;
    if(RadioButton16.Checked)then
      Label40.Caption:='起始字地址(Hex)'
    else
      Label40.Caption:='起始字节地址(Hex):';
    if (RadioButton20.Checked)then
    begin
      RadioButton13.Enabled:=False;
      RadioButton19.Enabled:=False;
...
begin
 if(Edit2.Text='')or(Edit3.Text='')then
  Exit;
    ...
    AddCmdLog('WriteData','块写', fCmdRet,ferrorcode);
    if fCmdRet=0 then
    begin
    StatusBar1.Panels.Items[0].text := FormatDateTime('hh:mm:ss', Now) +  ' “块写”指令返回=0x00' +
                '(数据完全写入成功)';
    end;
...
begin
  ...
  AddCmdLog('SetFhssMode', '设置跳频模式', fCmdRet);
end;
...
begin
  ...
  AddCmdLog('GetFhssMode', '获取跳频模式', fCmdRet);
end;
...
begin
  ...
  AddCmdLog('SetTriggerTime', '读取', fCmdRet);
end;
