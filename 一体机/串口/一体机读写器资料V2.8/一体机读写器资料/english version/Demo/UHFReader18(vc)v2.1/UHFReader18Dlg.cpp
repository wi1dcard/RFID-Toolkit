...
BOOL CUHFReader18Dlg::OnInitDialog()
{
	...
	m_list.InsertColumn(0,"EPC",LVCFMT_LEFT,320);
	...
	m_workmode.AddString("Answer Mode");
	m_workmode.AddString("Active Mode");
	m_workmode.AddString("Trigger Mode(Low)");
	m_workmode.AddString("Trigger Mode(High)");
	...
	m_quyu.AddString("Password");
    m_quyu.AddString("EPC");
	m_quyu.AddString("TID");
	m_quyu.AddString("User");
    m_quyu.SetCurSel(1);
...
//	port=strtoul(str,NULL,16);
	...
	{
		OnButton3();
		m_statusbar.SetPaneText(0,"Open Com Successed!");
		str.Format("%d",port);
	...
	{
		m_statusbar.SetPaneText(0,"Com is Opened!");
	}
	else
	{
		m_statusbar.SetPaneText(0,"Open Com Faield!");
...
void CUHFReader18Dlg::OnButton2() 
{
	...
	if(ret==0)
	{
		...
		m_statusbar.SetPaneText(0,"Closed Com Successed!");
		m_statusbar.SetPaneText(1,"");
	...
	{
		m_statusbar.SetPaneText(0,"Closed Com Faield!");
	}
}
...
void CUHFReader18Dlg::OnButton3() 
{
	...
	if (fCmdRet == 0)
	{
		...
		else
		{
			...
			m_statusbar.SetPaneText(0,"GetReaderInformation Successed!");
	}
	else
		m_statusbar.SetPaneText(0,"GetReaderInformation Faield!");
...
void CUHFReader18Dlg::OnButton4() 
{
	...
	if (fCmdRet == 0)
		m_statusbar.SetPaneText(0,"Set Successed!");
	else
	m_statusbar.SetPaneText(0,"Set Faield!");
...
void CUHFReader18Dlg::OnButton5() 
{
	...
	if (fCmdRet == 0) 
	{
		...
		m_statusbar.SetPaneText(0,"Get Successed!");
	}
	else
	m_statusbar.SetPaneText(0,"Get Faield!");
}
...
void CUHFReader18Dlg::OnButton6() 
{
    ...
    if (fCmdRet == 0) 
		returninfo = returninfo + ",Power set Successed";
    else
		returninfo = returninfo + ",Power set Faield";
	
    fCmdRet = RR_Writedfre(&ComAddr, &dmaxfre, &dminfre, FrmHandle);
    if (fCmdRet == 0) 
		returninfo = returninfo + ",Fre set Successed";
    else	
		returninfo = returninfo + ",Fre set Faield";
	
    fCmdRet = RR_Writebaud(&ComAddr, &fBaud, FrmHandle);
    if (fCmdRet == 0) 
		returninfo = returninfo + ",Baud set Successed";
    else
		returninfo = returninfo + ",Baud set Faield";
	
    fCmdRet = RR_WriteScanTime(&ComAddr, &ScanTime, FrmHandle);
    if (fCmdRet == 0) 
		returninfo = returninfo + ",ScanTime set Successed";
    else
		returninfo = returninfo + ",ScanTime set Faield";
...
void CUHFReader18Dlg::OnButton10() 
{
	...
	if(str.GetLength()<8)
	{
		MessageBox("Access Password Less Than 8 digit!Please input again£¡",NULL,MB_OK);
		return;
	}
	...
	if (fCmdRet == 0)
	{
		...
		m_statusbar.SetPaneText(0,"Read data Successed!");
	}
	else
	{
		m_statusbar.SetPaneText(0,"Read data Faield!");
	}
}
...
void CUHFReader18Dlg::OnButton11() 
{
	...
	{
		MessageBox("Please input Data in words in hexadecimal form!'+#13+#10+'For example: 1234¡¢12345678£¡",NULL,MB_OK);
		return;
	}
	...
	if (fCmdRet == 0)
	{
		m_statusbar.SetPaneText(0,"Write data Successed!");
	}
	else
	{
		m_statusbar.SetPaneText(0,"Write data Faield!");
	}
}
